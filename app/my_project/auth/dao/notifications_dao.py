...
class NotificationDAO:
    ...
    def insert_notification(self, data):
        # Перевірка для заборони вставки з timestamp, що закінчується на "00"
        if data['timestamp'].endswith('00'):
            raise Exception("Notifications timestamp cannot end with two zeros (INSERT)")

        cur = self.mysql.connection.cursor()
        cur.execute("INSERT INTO notifications (timestamp) VALUES (%s)", 
                    (data['timestamp'],))
        self.mysql.connection.commit()
        cur.close()

    def update_notification(self, notification_id, data):
        # Перевірка для заборони оновлення з timestamp, що закінчується на "00"
        if data['timestamp'].endswith('00'):
            raise Exception("Notifications timestamp cannot end with two zeros (UPDATE)")
