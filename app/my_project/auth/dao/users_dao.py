...
class UsersDAO:
    ...
    def insert_user(self, user_data):
        ...
        # Викликаємо збережену процедуру для вставки
        cur.callproc('insert_into_users', [
            user_data['user_id'], 
            user_data['access_id'], 
            user_data['username'], 
            user_data['email']
    ...
    def insert_noname_users_batch(self):
        cur = self.mysql.connection.cursor()
        try:
            # Виклик збереженої процедури без параметрів
            cur.callproc('insert_noname_users_batch')
            self.mysql.connection.commit()
